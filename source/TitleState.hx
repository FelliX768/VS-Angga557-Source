...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('anggaFreakyMenu'), 0);
			}
		}
		...
		logoBl.frames = Paths.getSparrowAtlas('anggaLogoBumpin');
	...
	override function beatHit()
	{
		...
		if (curBeat % 2 == 0)
		{
			...
			zoomTween = FlxTween.tween(FlxG.camera, {zoom: 1}, 0.4, {ease: FlxEase.circOut, onComplete: function(twn:FlxTween)
				{
					zoomTween = null;
				}
			});
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['The Creator Mod'], 15);
					#else
				...
				case 4:
					#if PSYCH_WATERMARKS
					addMoreText('Flumberly', 15);
					addMoreText('FelliX', 15);
					#else
					addMoreText('present');
					#end
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Angga557'); // credTextShit.text += '\nFunkin';
